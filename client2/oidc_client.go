...
const (
	clientID     = "hr_app" //OAuth Client ID
...
func main() {
	...
	mux.HandleFunc("/info", func(w http.ResponseWriter, r *http.Request) {
		auther.ClientConfig.Scopes = []string{oidc.ScopeOpenID, "address", "phone", "hr_read_info", "hr_edit_info", "hr_read_salary", "device_auth"}
	...
	mux.HandleFunc("/salary", func(w http.ResponseWriter, r *http.Request) {
		auther.ClientConfig.Scopes = []string{oidc.ScopeOpenID, "profile", "email", "hr_read_salary", "hr_edit_salary", "hr_read_info", "hr_edit_info", "device_trust"}
		logrus.Debugln("New authentication Request submitted from " + r.Host)
		http.Redirect(w, r, auther.ClientConfig.AuthCodeURL("statexyz"), http.StatusFound) //State Value is used to map Authorization Requests to Responses. Should probably be generated per Auth Request
	})
	...
	logrus.Fatal(http.ListenAndServe("127.0.0.1:5557", mux))
}
